...
void keccak_absorb(keccak_state_t *state, uint8_t rate, const uint8_t *data, size_t data_len) {
    uint8_t pos = state->pos;

    for (size_t i = 0; i < data_len; i++) {
        state->s[pos >> 3] ^= (uint64_t) data[i] << 8 * (pos % 8);
        pos++;
...
void keccak_absorb_nonce(keccak_state_t *state, uint8_t rate, uint32_t nonce) {
    uint8_t pos = state->pos;

    for (size_t i = 0; i < 4; i++) {
        state->s[pos >> 3] ^= (uint64_t) (nonce >> (8 * i)) << 8 * (pos % 8);
        pos++;
...
void keccak_finalize(keccak_state_t *state, uint8_t rate) {
    state->s[state->pos >> 3] ^= (uint64_t) 0x1f << 8 * (state->pos % 8);
    state->s[16] ^= 0x8000000000000000;

    state->pos = rate;
}

void keccak_squeeze(keccak_state_t *state, uint8_t rate, size_t data_len, uint8_t *data) {
    size_t i = 0;
    uint8_t pos = state->pos;
    ...
    while (data_len > 0) {
        ...
        if (data_len > rate) {
            memcpy(data + i, &state->s[pos >> 3], rate);
            pos += rate;
            i += rate;
            data_len -= rate;
        } else {
            memcpy(data + i, &state->s[pos >> 3], data_len);
            pos += data_len;
            data_len -= data_len;
        }
    }
